...
for i = 1:max_iter
    for j=1:rows
        for k=1:cols
            distance_from_centroid = zeros(no_of_robots);
            for l=1:no_of_robots
                distance_from_centroid(l) = sqrt(((j-centroids(l,1))^2)+((k-centroids(l,2))^2));
            end
            [M,classifications(j,k)] = min(distance_from_centroid);
        end
    end
    new_centroid = zeros(no_of_robots,2)
    new_loss = 0
    for m=1:no_of_robots
        same_voronoi_cell = find(classifications == m);
        new_centroid(m,:)=mean(same_voronoi_cell);
